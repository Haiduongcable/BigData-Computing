...
def login(persist_cookie):
    ...
    account = config['tiki']['account']
    password = config['tiki']['password']
    browser = Browser()
    try:
        browser.go(Proxy.login_page_url)
        browser.load_cookie_from(cookie_file)  #
        browser.send_by_css(Proxy.account_input_css_selector, account)
        browser.send_by_css(Proxy.password_input_css_selector, password)
        browser.click_by_xpath(Proxy.login_button_xpath)
        try:
            browser.wait_for(lambda driver: driver.find_element_by_css_selector(Proxy.verify_code_input_css_selector))
            browser.send_by_css(Proxy.verify_code_input_css_selector, input('input verify code: '))
            browser.click_by_xpath(Proxy.verify_button_xpath)
        except TE:
            pass
        browser.wait_for(lambda driver: driver.current_url == Proxy.home)
        if persist_cookie:
...
def main():
    ...
    cat_header = Proxy.general_header.copy()
    cat_header['method'] = 'GET'
    cat_header['path'] = path
    cat_header['cookie'] = cookie_string
    res = conn.get(Proxy.home + path, headers=cat_header)
    ...
    for cat in res.json()['categories']:
        try:
            ...
            attr_header = Proxy.general_header.copy()
            attr_header['method'] = 'GET'
            attr_header['path'] = path
            attr_header['cookie'] = cookie_string
            res = conn.get(Proxy.home + path, headers=attr_header)
            ...
            if 'modelid' in meta:
                ...
                item_header = Proxy.general_header.copy()
                ...
                item_header['origin'] = 'https://seller.tiki.tw'
                res = conn.post(Proxy.home + path, headers=item_header,
